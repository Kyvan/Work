FTPDownload.ps1
Connects to FTP server and downloads the two newest files based on time

setDKIM.ps1
Connects to Microsoft 365 (formerly Office 365) and sets DKIM for the tenant based on user input and then enables it

softwareInstalled.ps1
Gets triggered based on event logs when a new software is installed and sends an email with information about the software that was installed and who authorized the install

disablePasswordExpirationForAll.ps1
Disables passwords expiration for all users in an organization

disablePasswordExpiration.ps1
Disables password expiration for an individual user

permAndOwnershipTakeOver.ps1
Script to take ownership and change permissions of a file/folder