...
If ($IsDirSyncEnabled -eq $false) {
    ...
    Write-Host Please disable Active Directory Sync and Wait Exit
}
Start-Sleep -Seconds 5

#If you want to dump your existing AD to text file for reference uncomment the next line ldifde -f C:\export.txt -r (Userprincipalname=*) -l objectGuid, userPrincipalName
do {
    ...
    $Office365UserQuery = Get-MsolUser -UserPrincipalName $OnlineUser.UserPrincipalName | Select-Object DisplayName,ImmutableId Write-Host

    # Do the ID's Match? if not something is wrong Write-Host AD Immutable ID Used $UserimmutableID
